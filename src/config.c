							  ...
							  bool key_pressed, uint32_t mods_filtered, xkb_keysym_t sym)
{
	...
	for (size_t i = 0; i < cfg->n_binds; i++)
	{
		...
		case COMP_KEYBIND_QUIT:
			/* Route quit through the one-shot helper to avoid duplicate terminate races. */
			server_request_terminate(server, "Quit action requested, terminating display loop");
			return true;
