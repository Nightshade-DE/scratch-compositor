...
- **Crash handling:** stackcomp installs a minimal async-signal-safe fatal handler by default. Use **`--crash-log PATH`** to append short crash markers to a file, or **`--no-crash-handler`** to disable installation. See **`CRASHING.md`** for full core-dump workflow.
